...
void enumerate_configurations_r(uint_fast32_t rounds,
                                const card_t cards_per_round[],
                                uint_fast32_t round,
...
void enumerate_configurations(uint_fast32_t rounds,
                              const card_t cards_per_round[],
                              void (*observe)(uint_fast32_t,
                                              uint_fast32_t[],
                                              void*),
...
void tabulate_configurations(uint_fast32_t round,
                             uint_fast32_t configuration[],
                             void* data) {
...
out:
  ...
  for (uint_fast32_t i = 0; i < SUITS;) {
    ...
    for (uint_fast32_t j = 0, remaining = RANKS; j <= round; ++j) {
      uint_fast32_t ranks =
          configuration[i] >> ROUND_SHIFT * (indexer->numRounds() - j - 1) &
          ROUND_MASK;
...
void enumerate_permutations_r(uint_fast32_t rounds,
                              const card_t cards_per_round[],
                              uint_fast32_t round,
                              uint_fast32_t remaining,
                              uint_fast32_t suit,
...
void enumerate_permutations(uint_fast32_t rounds,
                            const card_t cards_per_round[],
                            void (*observe)(uint_fast32_t,
                                            uint_fast32_t[],
                                            void*),
...
void count_permutations(uint_fast32_t round,
                        uint_fast32_t count[],
                        void* data) {
  hand_indexer_t* indexer = data;
  ...
  for (uint_fast32_t i = 0; i <= round; ++i) {
    for (uint_fast32_t j = 0, remaining = indexer->cards_per_round()[i];
         j < SUITS - 1; ++j) {
      uint_fast32_t size =
          count[j] >> (indexer->numRounds() - i - 1) * ROUND_SHIFT & ROUND_MASK;
      idx += mult * size;
      mult *= remaining + 1;
      remaining -= size;
    }
...
void tabulate_permutations(uint_fast32_t round,
                           uint_fast32_t count[],
                           void* data) {
  hand_indexer_t* indexer = data;
...
bool hand_indexer_init(uint_fast32_t rounds,
                       const card_t cards_per_round[],
...
void hand_indexer_free(hand_indexer_t* indexer) {
  for (uint_fast32_t i = 0; i < indexer->numRounds(); ++i) {
...
hand_index_t hand_indexer_size(const hand_indexer_t* indexer,
                               uint_fast32_t round) {
  assert(round < indexer->numRounds());
...
bool hand_unindex(const hand_indexer_t* indexer,
                  ...
                  hand_index_t index,
                  card_t cards[]) {
  if (round >= indexer->numRounds() || index >= indexer->round_size[round]) {
    return false;
  }
